...
const loginUser = async ({email, password}) =>{const loggedUser = await User.findOne({email});
if(loggedUser) throw new Error("Email already in use"); // checking if a user with the same email already exists
...
const newAccessToken = jwt.sign({id: user._id, role: user.role}, process.env.ACCESS_TOKEN_SECRET, {expiresIn:"15m"}) // generate  new access token since the earlier one has expired
